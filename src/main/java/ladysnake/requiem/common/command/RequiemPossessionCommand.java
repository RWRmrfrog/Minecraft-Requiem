...
public final class RequiemPossessionCommand {
    ...
    static LiteralArgumentBuilder<ServerCommandSource> possessionSubcommand() {
        return literal(POSSESSION_SUBCOMMAND)
            ...
            .then(literal("stop")
                ...
                .executes(context -> stopPossession(context.getSource(), Collections.singleton(context.getSource().getPlayerOrThrow())))
            ...
            .then(literal("start")
                ...
                .then(argument("possessed", EntityArgumentType.entity())
                    .executes(context -> startPossession(context.getSource(), EntityArgumentType.getEntity(context, "possessed"), context.getSource().getPlayerOrThrow()))
    ...
    private static int startPossession(ServerCommandSource source, Entity possessed, ServerPlayerEntity player) {
        if (!(possessed instanceof MobEntity)) {
            throw new CommandException(new TranslatableTextContent("requiem:commands.possession.start.fail.not_mob", possessed.getDisplayName()));
        }

        if (!RemnantComponent.get(player).isIncorporeal()) {
            throw new CommandException(new TranslatableTextContent("requiem:commands.possession.start.fail.not_incorporeal", player.getDisplayName()));
        }

        boolean success = PossessionComponent.get(player).startPossessing((MobEntity) possessed);

        if (!success) {
            throw new CommandException(new TranslatableTextContent("requiem:commands.possession.start.fail", possessed.getDisplayName()));
        }
        ...
        if (source.getEntity() == player) {
            message = new TranslatableTextContent(baseKey + ".self", possessed.getDisplayName());
        } else {
            message = new TranslatableTextContent(baseKey + ".other", player.getDisplayName(), possessed.getDisplayName());
        }
    ...
    private static void sendStopPossessionFeedback(ServerCommandSource source, ServerPlayerEntity player, Entity formerHost) {
        if (source.getEntity() == player) {
            source.sendFeedback(new TranslatableTextContent("requiem:commands.possession.stop.success.self", formerHost.getDisplayName()), true);
        } else {
            source.sendFeedback(new TranslatableTextContent("requiem:commands.possession.stop.success.other", player.getDisplayName(), formerHost.getDisplayName()), true);
        }
    }
}
