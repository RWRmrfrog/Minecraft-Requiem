...
public final class RequiemEtherealCommand {
    ...
    static LiteralArgumentBuilder<ServerCommandSource> etherealSubcommand() {
        return literal(ETHEREAL_SUBCOMMAND)
            ...
            .then(literal("query")
                ...
                .executes(context -> queryEthereal(context.getSource(), context.getSource().getPlayerOrThrow()))
            ...
            .then(literal("set")
                ...
                .then(argument("ethereal", BoolArgumentType.bool())
                    .executes(context -> setEthereal(context.getSource(), Collections.singleton(context.getSource().getPlayerOrThrow()), BoolArgumentType.getBool(context, "ethereal")))
    ...
    private static int queryEthereal(ServerCommandSource source, ServerPlayerEntity player) {
        ...
        Text remnantState = new TranslatableTextContent("requiem:" + (remnant ? "ethereal" : "not_ethereal"));
        source.sendFeedback(new TranslatableTextContent("requiem:commands.query.success." + (source.getEntity() == player ? "self" : "other"), remnantState), true);
        return remnant ? 1 : 0;
    }
    ...
    private static int setEthereal(ServerCommandSource source, Collection<ServerPlayerEntity> players, boolean ethereal) {
        ...
        for (ServerPlayerEntity player : players) {
            if (RemnantComponent.get(player).isVagrant() != ethereal) {
                if (!isRemnant(player)) {
                    throw new CommandException(new TranslatableTextContent("requiem:commands.ethereal.set.fail.mortal", player.getDisplayName()));
                }
                if (!RemnantComponent.get(player).setVagrant(ethereal)) {
                    throw new CommandException(new TranslatableTextContent("requiem:commands.ethereal.set.fail", player.getDisplayName()));
                }
    ...
    private static void sendSetEtherealFeedback(ServerCommandSource source, ServerPlayerEntity player, boolean ethereal) {
        Text name = new TranslatableTextContent("requiem:" + (ethereal ? "ethereal" : "not_ethereal"));
        if (source.getEntity() == player) {
            source.sendFeedback(new TranslatableTextContent("requiem:commands.ethereal.set.success.self", name), true);
        } else {
            if (source.getWorld().getGameRules().getBoolean(GameRules.SEND_COMMAND_FEEDBACK)) {
                player.sendSystemMessage(new TranslatableTextContent("requiem:commands.ethereal.set.target", name), Util.NIL_UUID);
            }

            source.sendFeedback(new TranslatableTextContent("requiem:commands.ethereal.set.success.other", player.getDisplayName(), name), true);
        }
    }
