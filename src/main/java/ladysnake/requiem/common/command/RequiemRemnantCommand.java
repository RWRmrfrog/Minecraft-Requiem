...
public final class RequiemRemnantCommand {
    ...
    public static LiteralArgumentBuilder<ServerCommandSource> remnantSubcommand() {
        return literal(REMNANT_SUBCOMMAND)
            ...
            .then(literal("query")
                ...
                .executes(context -> queryRemnant(context.getSource(), context.getSource().getPlayerOrThrow()))
    ...
    private static int queryRemnant(ServerCommandSource source, ServerPlayerEntity player) {
        ...
        source.sendFeedback(new TranslatableTextContent("requiem:commands.query.success." + (source.getEntity() == player ? "self" : "other"), remnantState.getName(), player.getDisplayName()), true);
    ...
    private static void sendSetRemnantFeedback(ServerCommandSource source, ServerPlayerEntity player, RemnantType type) {
        if (source.getEntity() == player) {
            source.sendFeedback(new TranslatableTextContent("requiem:commands.remnant.set.success.self", type.getName()), true);
        } else {
            if (source.getWorld().getGameRules().getBoolean(GameRules.SEND_COMMAND_FEEDBACK)) {
                player.sendSystemMessage(new TranslatableTextContent("requiem:commands.remnant.set.target", type.getName()), Util.NIL_UUID);
            }

            source.sendFeedback(new TranslatableTextContent("requiem:commands.remnant.set.success.other", player.getDisplayName(), type.getName()), true);
        }
    }
}
