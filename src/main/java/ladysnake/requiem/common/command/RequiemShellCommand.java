...
public final class RequiemShellCommand {
    ...
    public static LiteralArgumentBuilder<ServerCommandSource> shellSubcommand() {
        return literal(SHELL_SUBCOMMAND)
            .then(literal("create")
                .executes(context -> RequiemCommand.runOne(context.getSource().getPlayerOrThrow(), player -> createShell(context.getSource().getPosition(), player)))
            ...
            .then(literal("split")
                ...
                .executes(context -> RequiemCommand.runOne(context.getSource().getPlayerOrThrow(), RequiemShellCommand::split))
            ...
            .then(literal("merge").then(argument("shell", entity())
                .executes(context -> {
                    ...
                    return RequiemCommand.runOne(context.getSource().getPlayerOrThrow(), player -> merge(player, shell));
    ...
    private static void merge(ServerPlayerEntity player, Entity entity) {
        if (!(entity instanceof PlayerShellEntity shell)) {
            throw new CommandException(new TranslatableTextContent("requiem:commands.shell.fail.not_shell"));
        }
    ...
    private static void split(ServerPlayerEntity player) {
        ...
        if (!remnantComponent.getRemnantType().isDemon()) {
            throw new CommandException(new TranslatableTextContent("pandemonium:commands.shell.split.fail.mortal", player.getDisplayName()));
        }
        if (remnantComponent.isVagrant()) {
            throw new CommandException(new TranslatableTextContent("pandemonium:commands.shell.split.fail.vagrant", player.getDisplayName()));
        }
