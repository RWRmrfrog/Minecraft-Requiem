...
public final class RequiemClientListener implements
        ...
        ClientTickEvents.START_CLIENT_TICK.register(client -> {
            if (client.player != null && client.targetedEntity instanceof FakeClientPlayerEntity fp && fp.getType() == RequiemEntities.PLAYER_SHELL) {
                if (PossessionComponent.getHost(client.player) != null && client.player.getUuid().equals(fp.getOwnerUuid())) {
                    client.inGameHud.setOverlayMessage(new TranslatableTextContent("requiem:merge_hint", FractureKeyBinding.etherealFractureKey.getBoundKeyLocalizedText()), false);
                    wasLookingAtShell.setTrue();
                }
            } else if (wasLookingAtShell.booleanValue()) {
                client.inGameHud.setOverlayMessage(LiteralTextContent.EMPTY, false);
    ...
    public void onTooltipBuilt(ItemStack item, @Nullable PlayerEntity player, @SuppressWarnings("unused") TooltipContext context, List<Text> lines) {
        if (player != null) {
            ...
            lines.add(new TranslatableTextContent(key).styled(style -> style.withColor(Formatting.DARK_GRAY)));
        }
    }
