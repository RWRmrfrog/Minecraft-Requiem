...
public class DemonSoulVesselItem extends Item {
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> lines, TooltipContext ctx) {
        lines.add(new TranslatableTextContent(tooltip).formatted(this.getTooltipColor()));
        ...
        if (stack.hasNbt()) {
            ...
            if (!StringHelper.isEmpty(author)) {
                lines.add((new TranslatableTextContent("book.byAuthor", author)).formatted(Formatting.GRAY));
            }
        }
    }
