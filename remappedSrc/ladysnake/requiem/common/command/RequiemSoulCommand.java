...
public final class RequiemSoulCommand {
    ...
    private static int removeSoul(ServerCommandSource source, Collection<? extends Entity> targets, boolean soulless) {
        ...
        for (Entity target : targets) {
            if (!(target instanceof LivingEntity living)) {
                throw new CommandException(new TranslatableTextContent("requiem:commands.soul.set.fail.not_living", target.getDisplayName()));
            }

            if (target.getType().isIn(RequiemCoreTags.Entity.SOULLESS)) {
                throw new CommandException(new TranslatableTextContent("requiem:commands.soul.set.fail.permanently_soulless"));
            }

            if (SoulHolderComponent.isSoulless(living) != soulless) {
                if (soulless) {
                    SoulHolderComponent.get(living).removeSoul();
                    source.sendFeedback(new TranslatableTextContent("requiem:commands.soul.remove.success", target.getDisplayName()), true);
                } else {
                    SoulHolderComponent.get(living).giveSoulBack();
                    source.sendFeedback(new TranslatableTextContent("requiem:commands.soul.restore.success", target.getDisplayName()), true);
                }
    ...
    private static int querySoul(ServerCommandSource source, Entity target) {
        ...
        source.sendFeedback(new TranslatableTextContent(message, target.getDisplayName()), true);
        return hasSoul ? 1 : 0;
    }
}
